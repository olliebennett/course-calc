...
<table id="required_grades">
	...
	<tbody>
		...
		<tr>
			<td><?= $grade_name; ?></td>
			<td><?= number_format(100*$grade,2); ?>%</td>
			<td><?php
...
<?php foreach ($calc['year'] as $year => $module) : ?>
		<tr>
			<td><?= $year; ?></td>
			<td><?= $calc['year'][$year]['credits_complete']; ?>/<?= $calc['year'][$year]['credits_complete']+$calc['year'][$year]['credits_incomplete']; ?></td>
			<td><?= (($calc['year'][$year]['year_completed_credit_weighting'] == 0) ? 'N/A' : number_format(100*($calc['year'][$year]['year_completed_mark_weighting'] / $calc['year'][$year]['year_completed_credit_weighting']),2).'%'); ?></td>		
		</tr>
<?php endforeach; ?>
	</tbody>
</table>
